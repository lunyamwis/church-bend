...
from ..models import (
    Book,Author,Tags,Category,Publisher
)


class BookValidations:
    '''Validations for the book data information that is entered'''

    def validate_book_data(self, kwargs):
        '''
        ...
        input_data['isbn']=kwargs.get('isbn',None)
        input_data['publisher']=kwargs.get('publisher',None)
        input_data['author']=kwargs.get('author',[])
        input_data['tags']=kwargs.get('tags',[])
        input_data['categories']=kwargs.get('categories',[])
        input_data['published']=kwargs.get('published',None)
        
        if input_data['publisher']:
            input_data['publisher'] = validate_object_id(
            input_data['publisher'], Publisher,
            "Publisher") 
        return input_data

    def validate_premium_book_data(self, kwargs):
        '''
